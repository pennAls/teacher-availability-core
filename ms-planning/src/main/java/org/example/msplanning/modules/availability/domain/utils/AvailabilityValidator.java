package org.example.msplanning.modules.availability.domain.utils;

import org.example.msplanning.modules.availability.domain.exceptions.InvalidAvailabilityException;
    ...
    public static void validate(DayOfWeek dayOfWeek, LocalTime startTime, LocalTime endTime) {
        ...
        if (dayOfWeek == DayOfWeek.SUNDAY) {
            throw new InvalidAvailabilityException(
                    "Não é permitido cadastrar disponibilidade aos domingos."
            );
        }
        ...
        if (durationInMinutes != 150) {
            throw new InvalidAvailabilityException(
                    "O período informado deve ter a duração exata de 2 horas e 30 minutos."
            );
        }
        ...
        if (dayOfWeek == DayOfWeek.SATURDAY &&
                startTime.isAfter(LocalTime.of(12, 0))) {
            throw new InvalidAvailabilityException(
                    "Aos sábados, apenas o turno da MANHÃ é permitido (07:00 ou 09:50)."
            );
        }
    }
}